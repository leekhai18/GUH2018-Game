...
namespace MarchingBytes
{
    ...
    public class EasyObjectPool : Singleton<EasyObjectPool>
    {
		...
		*/
        ...
        public GameObject GetObjectFromPool(string poolName, Vector3 position, Quaternion rotation, Transform parentTransform)
        {
            GameObject result = null;
            ...
            {
                Pool pool = poolDictionary[poolName];
                result = pool.NextAvailableObject(position, rotation, parentTransform);
                //scenario when no available object is found in pool
                if (result == null)
                {
                    Debug.LogWarning("No object available in pool. Consider setting fixedSize to false.: " + poolName);
        ...
        public void ReturnObjectToPool(GameObject go)
        {
            ...
            else
            {
                if (poolDictionary.ContainsKey(po.poolName))
                {
                    ...
                    Debug.Log("Return to " + po.poolName);
                }
                else
                {
